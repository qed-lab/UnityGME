...
namespace Mediation.PlanTools
{
	...
	public class State : IState
	{
		...
		// Adds the given literal to the state.
		public State AddLiteralToState(IPredicate literal)
		{
			// Check this state's predicates to see if it is already there
			if (!this.InState(literal))
			{
				// Create a new state object.
				State state = Clone() as State;

				// if not, clone this state, add the literal and return it.
				List<IPredicate> literals = state.Predicates;
				literals.Add(literal);
				state.Predicates = literals;
				return state;
			}
			else
			{
		...
		// Removes the given literal from the state.
		public State RemoveLiteralFromState(IPredicate literal)
		{
			// Check this state's predicates to see if it is there.
			if (this.InState(literal))
			{
				// Create a new state object.
				State state = Clone() as State;

				// Remove the literal and return the state.
				List<IPredicate> literals = state.Predicates;
